...
- the way to log a metric and a parameter on [hosted_mlflow server]({mlflow-server-url-provided-at-lecture-time}/#/experiments/0)
...
You will all log you run results and parameters on the same MLFLOW instance hosted on the MLFlow server.
...
- View results with on [hosted_mlflow server]({mlflow-server-url-provided-at-lecture-time}/#/experiments/0)
