...
An example of Hosted MLFlow server is "https://user:PASSWORD@mlflow-dev.herokuapp.com/"

For simplicity purpose, we will just provide the url of the tunneled MLFlow running on my machine. The URL will be provided at lecture time.
...
mlflow.set_tracking_uri("{mlflow-server-url-provided-at-lecture-time}")
client = MlflowClient()
try:
    experiment_id = client.create_experiment(EXPERIMENT_NAME)
except BaseException:
...
```
Check out the mlflow server to visualise your logs
...
```
- check it logged correctly  on the URL (to be provided at the time of the coures)
